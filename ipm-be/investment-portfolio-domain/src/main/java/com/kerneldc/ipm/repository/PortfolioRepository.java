...
public interface PortfolioRepository extends BaseTableRepository<Portfolio, Long> {
	...
	@Query(value = """
			select p.id, p.financial_institution as financialInstitutionNumber, p.name, p.holder, p.account_number as accountNumber, p.currency, p.logically_deleted as logicallyDeleted,
